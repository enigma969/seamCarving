...
public class SeamCarvingNew {
    ...
    public static void seamCarving(double[][] cumulativeEnergyArray, BufferedImage img) {
        ...
        for (int n = 0; n < 300; ++n) {
            ...
            imgPainted = drawPathOnImage(imgPainted, path);
        }
        showImage(img);
        showImage(imgPainted);
    }
    
    private static void copyImg(BufferedImage src, BufferedImage target) {
    ...
    public static double[][] getCumulativeEnergyArray(BufferedImage img) {
        ...
        for (int y = 1; y < height; ++y) {
            for (int x = 1; x < width - 1; ++x) {
                cumulative_energy_array[y][x] = getGrayValue(x, y, img);
            }
        }
        ...
        for (int y = 1; y < height; ++y) {
            for (int x = 1; x < width - 1; ++x) {
                ...
                temp = getMinValue(tempArray3) + getGrayValue(x, y, img);
